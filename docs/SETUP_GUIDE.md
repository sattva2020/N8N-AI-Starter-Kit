...
### Поведение интерактивного генератора `.env`

В интерактивном режиме теперь, если файл `.env` уже существует, скрипт автоматически создаёт его резервную копию (в папке `backup/`) и продолжает выполнение без повторного подтверждения от пользователя. Это убирает повторяющиеся запросы при последовательных запусках скрипта.
