...
> Примечание: В некоторых сборках/ролях UI раздел Personal access tokens может быть скрыт или недоступен. В таком случае используйте Public API ключ (см. ниже) и эндпоинты `/api/v1/...` вместо `/rest/...`.
