...
if [ "$SETUP_MODE" = "template" ]; then
  ...
  if [ -f .env ]; then
    print_warning "Файл .env уже существует. Создаю резервную копию и продолжаю (без запроса)."
    # Автоматически создаём резервную копию, чтобы не запрашивать повторно у пользователя
    if ! backup_existing_config; then
      print_warning "Не удалось создать резервную копию .env — продолжаем без резервной копии"
    fi
