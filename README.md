...
### Сетевые рекомендации и экспозиция сервисов

Мы добавили документацию с примером патча, гарантирующего, что Traefik, n8n и Ollama подключаются к одним и тем же сетям — это уменьшает шанс 504 после перезапуска контейнеров: `docs/compose_network_patch.md`.
...
Инструмент для автоматизации: `scripts/create_n8n_credential.sh` — утилита для создания credential в n8n через REST API (использует admin token). См. `docs/credentials.md` для примеров использования.

## Автоматическое создание credential при старте

Скрипт `start.sh` может опционально запустить `scripts/create_n8n_credential.sh` во время старта стека и автоматически создать заранее заданные учетные данные (например, для Qdrant). Это поведение по умолчанию отключено и должно быть явно включено в `.env`.

Ключевые моменты:

- Переменная: `N8N_AUTO_CREATE_CREDENTIALS` — если установлена в `true`, `start.sh` попытается запустить `./scripts/create_n8n_credential.sh` (только если скрипт исполняемый и доступен в корне репозитория).
- Скрипт читает настройки из `.env` (если вы используете `--env-file` или `.env` в корне). Для известных типов credential (например, `qdrantApi`) `create_n8n_credential.sh` попытается собрать поля автоматически из соответствующих переменных окружения (`QDRANT_URL`, `QDRANT_API_KEY`).
- Безопасность: для взаимодействия с API n8n требуется `N8N_ADMIN_TOKEN` (или `N8N_TOKEN`). Никогда не коммитьте реальные токены в репозиторий. `start.sh` и `scripts/create_n8n_credential.sh` ожидают, что токен доступен через `.env` или передается в окружении во время выполнения.
- Идемпотентность и флаги: `create_n8n_credential.sh` поддерживает `--force` (принудительное создание/игнорирование строгой валидации) и `--bulk-file` для массового создания из CSV/JSON. При использовании в `start.sh` рекомендуется сначала протестировать создание вручную с `--dry-run` (если вы используете локальную версию импортера) либо запускать на отдельном тестовом инстансе.

Пример минимального блока в `.env` для автосоздания Qdrant credential:

```env
# Включить автоматическое создание credential при старте (по умолчанию — false)
N8N_AUTO_CREATE_CREDENTIALS=true

# Токен администратора n8n (не коммитить в репозиторий!)
N8N_ADMIN_TOKEN=changeme_super_secret_token

# Параметры для Qdrant (используются create_n8n_credential.sh для сборки payload)
QDRANT_URL=http://qdrant:6333
QDRANT_API_KEY=
```

Как это работает в `start.sh`:

- Если `N8N_AUTO_CREATE_CREDENTIALS=true` и `./scripts/create_n8n_credential.sh` исполняемый, `start.sh` выполнит скрипт перед автo-импортом workflows.
- Если локальный скрипт не найден или не исполняемый — `start.sh` пропустит шаг (без ошибок) и продолжит обычную последовательность запуска.
