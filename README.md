# N8N AI Starter Kit — self-hosted стек для локальной разработки AI

Небольшой, но мощный набор инструментов для быстрого развёртывания локальной
среды AI‑разработки на основе n8n и популярных компонентов (Ollama, Qdrant,
Postgres, MinIO и т.д.). Репозиторий содержит scripts, примеры workflows и
документацию по безопасной работе с учетными данными.

Подробнее в `docs/SETUP_SCRIPT.md` и `docs/`.
...
- Добавлены настройки `N8N_PROBE_HOST` и `N8N_READY_TIMEOUT` в `env.schema` —
  позволяют управлять локальными probe для n8n и увеличить таймаут ожидания
  готовности в медленных окружениях (подробности в `env.schema`).

- Обновление инфраструктуры: добавлены Traefik-лейблы для сервиса `n8n` в
  `docker-compose.yml` (маршрутизация использует переменную `N8N_DOMAIN`,
...
Добавлена вспомогательная функция в `scripts/setup.sh` для централизованной генерации пароля Neo4j и новый тестовый скрипт `scripts/test_env_gen.sh` для проверки генерации `.env` в режиме `--generate-only`.
...
сервиса `n8n` — теперь используется Node.js inline probe вместо `curl` внутри
контейнера, чтобы избежать зависимостей от дополнительных пакетов в образе.
Также Traefik-лейблы указывают на использование переменной `${N8N_DOMAIN}`
и entrypoint `websecure` с `certresolver=myresolver`.
Если вы вносите изменения в `docker-compose.yml` или связанные infra-файлы,
пожалуйста, обновите соответствующие разделы документации (`README.md` или
файлы в `docs/`) чтобы pre-commit хуки не блокировали коммиты.

Временно: включено DEBUG логирование Traefik для отладки регистраций маршрутов
и провайдеров (этот флаг можно убрать после сбора логов). Это изменение
добавлено в `docker-compose.yml` и задокументировано здесь.
...
В этом проекте собраны сервисы для локальной разработки и небольших production-нагрузок: оркестрация рабочих процессов, локальные LLM, векторное хранилище, БД и вспомогательные сервисы.

- n8n — оркестратор рабочих процессов и интеграций (REST API). Центр автоматизации, где запускаются workflow и обрабатываются триггеры.
- Ollama — локальный LLM‑движок для генерации и тестирования моделей без отправки данных наружу.
- Qdrant — векторное хранилище для embeddings и semantic search (используется в RAG-пайплайне).
- PostgreSQL (+pgvector) — основная реляционная база данных для метаданных и состояния n8n.
- MinIO — S3-совместимое хранилище для бинарных артефактов и загружаемых файлов.
- Neo4j + Graphiti — графовая память и контекст для долгосрочной памяти и связных данных.
- Web Interface, Document Processor — FastAPI‑сервисы для управления документами и их векторизации.
- Traefik — обратный прокси и маршрутизатор для внешнего доступа к сервисам.

Полный список сервисов приведён выше и в `docker-compose.yml` — часть сервисов может быть отключена через комментарии или профили.
...
- JupyterLab — интерактивная среда для отладки и анализа.
- Prometheus / Grafana / Loki / cAdvisor — мониторинг и логирование (опционально для профиля `developer`).

Рекомендации и ссылки

- Для автоматического создания credentials используйте `--dry-run` перед реальным импортом.
- Bulk‑файлы: отдавайте предпочтение JSON; если используете CSV — следуйте подсказкам в `docs/credentials_bulk.md`.
- Скрипты управления и импорта находятся в каталоге `scripts/` и `n8n/workflows/management/`.

> [!NOTE]
> Профиль `cpu` содержит минимальный набор сервисов для локальной работы n8n с LLM; профиль `developer` включает дополнительные инструменты для разработки и отладки.
- Тщательно протестируйте процесс в staging перед применением в production — автоматическое создание учетных данных изменяет состояние инстанса n8n.
- Для массового создания credential используйте `scripts/create_n8n_credential.sh --bulk-file credentials.csv` (см. `scripts/create_n8n_credential.sh` для формата CSV/JSON).
- Если вы предпочитаете полностью ручной контроль — оставьте `N8N_AUTO_CREATE_CREDENTIALS=false` и создайте credential вручную через интерфейс n8n или запустите скрипт локально при необходимости.
...
Минимальные шаги для локального запуска (developer):
...
# 2. Сгенерировать .env (интерактивно или скриптом)
./scripts/setup.sh

# 3. Запустить стек (developer profile)
docker compose --profile developer up -d --build

# 4. Проверить сервисы:
# n8n: http://localhost:5678
# web-interface (если включён): http://localhost:8002
# qdrant: http://localhost:6333
```

Советы:
- Используйте `--dry-run` при тестировании `scripts/create_n8n_credential.sh`.
- Для bulk‑операций используйте JSON-файлы — CSV поддерживается как fallback.
...
## 🪝 Настройка pre-commit hooks (рекомендуется)

Мы используем `pre-commit` для локальной проверки `env.schema`, форматирования и поиска очевидных плейсхолдеров перед коммитом.

Чтобы включить хуки локально для разработчика выполните:

```bash
# установить pre-commit (требуется Python и pip)
pip install --user pre-commit

# в корне репозитория установить хуки
pre-commit install

# опционально прогон всех хуков один раз
pre-commit run --all-files
```

Если вы используете Windows + Git Bash, убедитесь, что Git настроен на использование LF для строковых переводов (`git config core.autocrlf false`) чтобы избежать предупреждений про CRLF.

Хуки включают проверку на пустые/placeholder значения в `.env` и форматирование документации. Не коммитьте реальные `.env` и секреты.
