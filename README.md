...
> [!NOTE]
> При использовании профиля `minimal` запускаются только основные компоненты, необходимые для работы n8n с локальным ИИ. Профиль `developer` включает дополнительные инструменты для разработки.
