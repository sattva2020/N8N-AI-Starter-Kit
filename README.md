...
Примечание: если `--expand-env` недоступен (нет python3) или развёртывание плейсхолдеров не прошло, скрипт продолжит с исходным JSON и выведет предупреждение, чтобы импорт не прерывался.
