...
[Подробнее о майском обновлении](./docs/MAY_2025_UPDATE.md) | [Решение проблемы с хешем пароля Traefik](./docs/TRAEFIK_PASSWORD_FIX.md)
...
> [!NOTE]
> Если вы столкнетесь с проблемами во время установки, обратитесь к [руководству по устранению неполадок](TROUBLESHOOTING.md).
...
N8N AI Starter Kit поддерживает несколько профилей запуска, чтобы адаптировать систему под различные потребности и оборудование:

| Профиль       | Описание                                                                                 | Команда запуска                            |
|---------------|------------------------------------------------------------------------------------------|-------------------------------------------|
| `cpu`         | Стандартная конфигурация для CPU (по умолчанию)                                          | `docker compose --profile cpu up -d`       |
| `gpu-nvidia`  | Конфигурация для использования GPU Nvidia, ускоряет работу с языковыми моделями          | `docker compose --profile gpu-nvidia up -d` |
| `gpu-amd`     | Конфигурация для использования GPU AMD (только Linux)                                    | `docker compose --profile gpu-amd up -d`    |
| `minimal`     | Минимальная конфигурация, включает только основные сервисы для работы n8n с локальным ИИ | `docker compose --profile minimal up -d`    |
| `developer`   | Расширенная конфигурация с инструментами мониторинга, аналитики и разработки             | `docker compose --profile developer up -d`  |
