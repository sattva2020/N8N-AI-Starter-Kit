...
class ClickHouseService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                "ClickHouse service initialized",
                host=self.config.clickhouse_host,
                database=self.config.clickhouse_database,
            )
    ...
    async def execute(self, query: str, params: dict | None = None) -> list[Any]:
        ...
        try:
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(None, lambda: self.client.execute(query, params or {}))
            return result
    ...
    async def get_workflow_analytics(
        self, start_date: date, end_date: date, workflow_id: str | None = None
    ) -> WorkflowAnalytics:
        ...
        try:
            ...
            total_workflows_query = (
                f"SELECT count(DISTINCT workflow_id)\nFROM workflow_executions\nWHERE {where_clause}\n"
            )
            ...
            executions_by_day = [
                {"date": str(row[0]), "executions": row[1], "successful": row[2], "failed": row[3]}
            ...
            # Failure rate trend
            failure_rate_trend_query = (
                "SELECT\n"
                "    toDate(started_at) as date,\n"
                "    CASE WHEN count(*) > 0 THEN "
                "countIf(status = 'error') * 100.0 / count(*) "
            ...
            failure_rate_trend = [
                {"date": str(row[0]), "failure_rate": float(row[1])} for row in failure_rate_trend_data
            ]
    ...
    async def get_top_performing_workflows(
        ...
        try:
            query = (
                ...
                "    countIf(status = 'success') as successful_executions,\n"
                "    CASE WHEN count(*) > 0 THEN "
                "countIf(status = 'success') * 100.0 / count(*) ELSE 0 END as success_rate,\n"
                "    COALESCE(avg(duration_ms), 0) as avg_duration_ms\n"
                "FROM workflow_executions\n"
            ...
            result = await self.execute(query, {"start_date": start_date, "end_date": end_date, "limit": limit})
    ...
    async def get_user_activity(
        self, start_date: date, end_date: date, _user_id: str | None = None, _offset: int = 0, _limit: int = 100
    ) -> UserActivity:
    ...
    async def get_system_metrics(
        self, start_time: datetime, end_time: datetime, _metric_type: str | None = None
    ) -> SystemMetrics:
        """Get system metrics"""
        try:
    ...
    async def get_document_analytics(
        self, start_date: date, end_date: date, _document_type: str | None = None
    ) -> DocumentAnalytics:
    ...
    async def get_api_usage_stats(
        self, start_date: date, end_date: date, _endpoint: str | None = None
    ) -> APIUsageStats:
    ...
    async def get_error_analysis(
        self, start_date: date, end_date: date, error_type: str | None = None, workflow_id: str | None = None
    ) -> ErrorAnalysis:
        """Get error analysis"""
        try:
            ...
            errors_by_type = [{"error_type": row[0], "count": row[1]} for row in errors_by_type_data]
    ...
    async def get_realtime_dashboard_data(self, start_time: datetime, end_time: datetime) -> RealtimeDashboard:
        ...
        try:
            ...
            """
            active_workflows = (
                await self.execute(active_workflows_query, {"start_time": start_time, "end_time": end_time})
            )[0][0]
            ...
            """
            current_executions = (
                await self.execute(current_executions_query, {"start_time": current_start, "end_time": end_time})
            )[0][0]
            ...
            success_rate_query = """
                ...
                WHERE started_at >= %(start_time)s AND started_at <= %(end_time)s
            """
            success_rate_result = await self.execute(
                success_rate_query, {"start_time": start_time, "end_time": end_time}
            )
