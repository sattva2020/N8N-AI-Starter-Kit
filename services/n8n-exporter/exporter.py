...
class N8NMetricsExporter:
    ...
    def __init__(self):
        ...
        # Метрики N8N
        self.n8n_info = Info('n8n_info', 'Basic N8N instance info', registry=self.registry)

        # Counters / Gauges used by exporter (defined early so make_api_request can use them)
        self.workflows_total = Gauge('n8n_workflows_total', 'Total workflows count', registry=self.registry)
        self.workflows_active = Gauge('n8n_workflows_active', 'Active workflows count', registry=self.registry)

        self.executions_total = Counter(
            'n8n_executions_total', 'Total workflow executions', ['workflow_name', 'status'], registry=self.registry
        )
        ...
        self.execution_duration = Histogram(
            'n8n_execution_duration_ms', 'Workflow execution duration ms', ['workflow_name'], registry=self.registry
        )

        self.nodes_total = Gauge('n8n_nodes_total', 'Total number of nodes', ['node_type'], registry=self.registry)

        self.credentials_total = Gauge(
            'n8n_credentials_total', 'Total number of credentials', ['credential_type'], registry=self.registry
        )

        # Метрики производительности API
        self.api_requests_total = Counter(
            'n8n_api_requests_total', 'Total API requests', ['endpoint', 'method', 'status'], registry=self.registry
        )
    ...
    def make_api_request(self, endpoint: str, method: str = 'GET') -> dict | None:
        ...
        try:
            ...
            self.api_requests_total.labels(endpoint=endpoint, method=method, status=response.status_code).inc()
            ...
            self.api_requests_total.labels(endpoint=endpoint, method=method, status='error').inc()
    ...
    def collect_execution_metrics(self):
        ...
        try:
            ...
            for execution in executions:
                ...
                # Обновляем счетчики
                self.executions_total.labels(workflow_name=workflow_name, status=status).inc()

                # Если выполнение провалилось
                if status == 'error':
                    error_type = execution.get('data', {}).get('resultData', {}).get('error', {}).get('name', 'unknown')
                    self.executions_failed_total.labels(workflow_name=workflow_name, error_type=error_type).inc()
                ...
                if start_time_str and stop_time_str:
                    try:
                        ...
                        self.execution_duration.labels(workflow_name=workflow_name).observe(duration_ms)
                    except Exception as e:
    ...
    def collect_system_info(self):
        ...
        try:
            ...
            if info_data:
                ...
                # Fallback информация
                self.n8n_info.info({'version': 'unknown', 'instance_id': 'unknown', 'exporter_version': '1.0.0'})
